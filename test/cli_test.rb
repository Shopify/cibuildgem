    ...
    def test_package_single_abi_runs_rake_tasks_for_each_ruby_version
      ...
      cli.stub(:compilation_task, compilation_task) do
        cli.stub(:run_rake_tasks!, run_rake_tasks) do
          Dir.stub(:glob, find_built_gems) do
            cli.package
          end
        end
      end
      ...
      assert_equal(["pkg/3.3/*.gem", "pkg/3.2/*.gem"], gem_globs)
    end

    def test_package_single_abi_fails_when_no_single_abi_gem_is_built
      cli = CLI.new([], { "include-single-abi" => true })
      compilation_task = Struct.new(:ruby_cc_version, :ruby_versions).new(
        "3.3.8",
        [Gem::Version.new("3.3.8")],
      )

      error = cli.stub(:compilation_task, compilation_task) do
        cli.stub(:run_rake_tasks!, nil) do
          Dir.stub(:glob, []) do
            assert_raises(RuntimeError) { cli.package }
          end
        end
      end

      assert_equal(
        "Could not build a single-ABI gem for Ruby 3.3. Ensure RubyGems and rake-compiler support content-addressable gems.",
        error.message,
      )
    end
