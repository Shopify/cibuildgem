    ...
    def package
      ...
      if options["include-single-abi"] == true
        compilation_task.ruby_versions.each do |ruby_version|
          ...
          ruby_abi = ruby_version.segments.first(2).join(".")
          next if Dir.glob(File.join("pkg", ruby_abi, "*.gem")).any?

          raise "Could not build a single-ABI gem for Ruby #{ruby_abi}. Ensure RubyGems and rake-compiler support content-addressable gems."
        end
      end
    end
